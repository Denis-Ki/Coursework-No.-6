...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        ...
        moderator_group.permissions.add(
            can_unpublish_product, change_product_description, change_product_category
        )

        self.stdout.write(
            self.style.SUCCESS(
                'Группа "Модераторы" и необходимые разрешения созданы успешно.'
            )
        )
        self.stdout.write(
            self.style.SUCCESS('Добавьте пользователей группе "Модераторы".')
        )
